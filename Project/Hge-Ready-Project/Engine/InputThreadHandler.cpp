...
DWORD WINAPI InputThread(void* threadData)
{
	...
	for(;;)
	{
		...
		{
			inputThreadData->myIsDoneFlag = false;

			inputThreadData->myInput->Update();

			InputData inputStuff = inputThreadData->myInput->GetData();
	
			GrowingArray<InputData>& currentArrayToSpam = (*inputThreadData->myInputUpdates)[static_cast<int>(inputThreadData->myCurrentArrayToSpam)];
			currentArrayToSpam.Add(inputStuff);
